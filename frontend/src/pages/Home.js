...
import { Box, Typography, TextField, Button, Alert, InputAdornment, IconButton, Grid, Card, CardMedia, CardContent, CardActionArea, Autocomplete } from '@mui/material';
...
const Home = ({ user, resetHome, greeting }) => {
  ...
  return (
    <Box maxWidth={900} mx="auto" mt={6} p={3} bgcolor="white" borderRadius={2} boxShadow={2}>
      <style>{`
      ...
      `}</style>
      {user && greeting && (
        <Typography variant="h6" mb={2} fontWeight={700} align="center" color="secondary">
          {greeting}
        </Typography>
      )}
      <Autocomplete
      ...
      {selectedDish && (
        <form onSubmit={handleFetch}>
          ...
          <Box display="flex" alignItems="center" justifyContent="center" gap={1} mb={2}>
            <IconButton onClick={handleMinus} size="medium" sx={{ border: '1px solid #ccc', borderRadius: '50%', bgcolor: '#fafafa', transition: '0.2s', '&:hover': { bgcolor: '#f0f0f0', borderColor: 'primary.main' } }}>
              <RemoveIcon />
            </IconButton>
            <TextField
              type="number"
              ...
              sx={{ width: 80 }}
            />
            <IconButton onClick={handlePlus} size="medium" sx={{ border: '1px solid #ccc', borderRadius: '50%', bgcolor: '#fafafa', transition: '0.2s', '&:hover': { bgcolor: '#f0f0f0', borderColor: 'primary.main' } }}>
              <AddIcon />
            </IconButton>
          </Box>
